...
link_locations <- data.table(matrix(ncol = 4, nrow = nrow(wells2010)))
...
link_locations$Production.Type <- wells2010$Production_Type
...
inv_distancer <- function( n, 
  ...
  out <- data.table( N = n,
                     ...
                     grid.sf[, c( 'statefp10', 'countyfp10', 'name10', 'geometry')],
...
exp_inverse_dist_sum <- 
  exp_inverse_dist[, .( exposure = sum( exp_inv_dist)),
                   by = .( Production.Type, countyfp10)] %>%
  merge( roadiness.trans.county, by = c('statefp10', 'countyfp10'))
