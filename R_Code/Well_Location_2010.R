...
wells2010 <-  read.csv(file = '/projects/HAQ_LAB/xshan2/R_Code/renv/ActiveWells_US_2010.csv')
...
#production types include: Oil, Gas, O&G
CAwells2010 <- wells2010[wells2010$Production_Type==Oil,]
link_locations <- data.table(matrix(nrow = nrow(CAwells2010), ncol = 3))
link_locations <- setnames(link_locations,c("ID","longitude","latitude"))
link_locations$ID <- 1:nrow(CAwells2010)
link_locations$longitude <- CAwells2010$Longitude
link_locations$latitude <- CAwells2010$Latitude

#link_locations <- data.table(matrix(nrow = nrow(wells2010), ncol = 3))
#link_locations <- setnames(link_locations,c("ID","longitude","latitude"))
#link_locations$ID <- 1:nrow(wells2010)
#link_locations$longitude <- wells2010$Longitude
#link_locations$latitude <- wells2010$Latitude
